...
const serializeSkill = (skill, language = 'en') => {
  ...
  const name = getText(skill.name, language) || 
               getText(skill.skillData?.name, language) || 
               getText(skill.skillTypeText, language) || 
               'Compétence';

  const rawType = getText(skill.skillTypeText, language) || 
                  (skill.skillType && (skill.skillType.id || skill.skillType.name) ? skill.skillType : null);
  ...
  const level = (() => {
    ...
    if (skill.level && typeof skill.level === 'object') {
      return Number(skill.level.level || skill.level.rank || skill.level.value || 1);
    }
    return 1;
  })();

  const desc = getText(skill.simpleDescription, language) || 
               getText(skill.description, language) || 
               '';

  const icon = normalizeImageAsset(skill.icon || skill.skillIcon || skill.skillData?.icon);
