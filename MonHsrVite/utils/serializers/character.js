...
const serializeSkillTree = (skillTrees, skillsArray) => {
  ...
  const getSkillDesc = (stId) => {
    ...
    const baseSkill = skillIndex[alt] || null;
    if (!baseSkill) return null;

    const relatedSkills = (skillsArray || []).filter(
      (s) => s.type === baseSkill.type,
    );
    if (relatedSkills.length > 1) {
      const combinedDesc = relatedSkills
        .map(
          (s) =>
            `<span style="color:#d8b467; font-weight:bold;">${s.name}</span><br/>${s.desc || s.simple_desc}`,
        )
        .join("<br/><br/>");
      return { ...baseSkill, desc: combinedDesc };
    }
    return baseSkill;
  };
