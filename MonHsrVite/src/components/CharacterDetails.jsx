...
const sanitizeAndFormatDescription = (text) => {
  ...
  const cleaned = String(text)
    .replace(/<\/?u>/gi, "").replace(/<\/?unbreak>/gi, "").replace(/<\/?i>/gi, "")
    .replace(/\{F#([^}]*)\}\{M#[^}]*\}/gi, "$1")
    .replace(/\{M#([^}]*)\}\{F#[^}]*\}/gi, "$1")
    .replace(/\{[FM]#([^}]*)\}/gi, "$1")
    .replace(/\s+/g, " ").trim();
  ...
  return lines.map((line, li) => {
    ...
    while ((m = colorRegex.exec(line)) !== null) {
      ...
      parts.push(<span key={`c-${li}-${m.index}`} style={{ color: m[1].substring(0,7), fontWeight:"bold" }}>{m[2]}</span>);
      last = colorRegex.lastIndex;
    }
    if (last < line.length) parts.push(line.substring(last));
    return <span key={`l-${li}`}>{parts.length ? parts : line}{li < lines.length - 1 && <br />}</span>;
  });
};
          ...
          <div className="column is-narrow">
            {charImages?.icon ? (
              <figure className="image is-64x64">
                <img src={charImages.icon} alt={activeCharacter.name}
            ...
            <div className="box equipment-box">
              ...
              {activeCharacter.lightCone?.effectDescription && (
                <div className="mt-4 p-3 rounded has-background-black-bis has-border-left-hsr">
                  <h5 className="title is-6 has-text-gold mb-2">{sanitizeName(activeCharacter.lightCone.name)}</h5>
...
const STAT_ICONS = {
  hp:"❤️", atk:"⚔️", def:"🛡️", spd:"💨", crit_rate:"🎯", crit_dmg:"💥",
  break_dmg:"⚡", effect_hit:"🎲", effect_res:"🔮", heal_rate:"💚",
  fire_dmg:"🔥", ice_dmg:"❄️", thunder_dmg:"⚡", wind_dmg:"🌀",
  quantum_dmg:"🌌", imaginary_dmg:"✨", elation_dmg:"✨", physical_dmg:"👊",
};
const STAT_COLORS = {
  hp:"#e05555", atk:"#e08c30", def:"#4fa3d1", spd:"#7ecba1",
  crit_rate:"#d4a0e0", crit_dmg:"#e060a0", break_dmg:"#e0c040",
  effect_hit:"#80b0e0", effect_res:"#9070d0", heal_rate:"#50c060",
};

function StatsPanel({ stats }) {
  if (!stats?.length) return <p className="has-text-grey-light is-size-7">Aucune statistique.</p>;
  return (
        ...
        return (
          <div key={stat.key} style={{
            ...
            <div style={{ display:"flex", alignItems:"center", gap:"7px" }}>
              <span style={{ fontSize:"0.9rem", lineHeight:1 }}>{STAT_ICONS[stat.key] || "📊"}</span>
              <span style={{ flex:1, fontSize:"0.68rem", color:"#b3b3b3", fontFamily:"Orbitron,sans-serif", textTransform:"uppercase" }}>
                {stat.name}
              </span>
              <span style={{ fontSize:"0.9rem", fontWeight:700, fontFamily:"Orbitron,sans-serif", color }}>
                {stat.total}
              </span>
            </div>
            {stat.base && stat.bonus && (
              <div style={{ display:"flex", justifyContent:"flex-end", gap:"8px", fontSize:"0.63rem", color:"#666", paddingLeft:"24px", marginTop:"2px" }}>
                <span>base <span style={{ color:"#aaa" }}>{stat.base}</span></span>
                <span style={{ color:"#555" }}>+</span>
                <span>équip. <span style={{ color:"#d8b467" }}>{stat.bonus}</span></span>
              </div>
            )}
          </div>
        );
